...
export class FlagsController {
  ...
  @ApiResponse({ status: 201, description: 'Flag created successfully', type: Flag })
  ...
  async findAll(
    ...
    if (submissionId) {
      return this.flagsService.findBySubmissionId(submissionId);
    }
    if (questionId) {
  ...
  @ApiResponse({ status: 200, description: 'Flag updated successfully', type: Flag })
  @ApiResponse({ status: 404, description: 'Flag not found' })
  async update(@Param('id') id: string, @Body() updateDto: UpdateFlagDto): Promise<Flag> {
